  ...
  switch (action) {
    case 'list': {
      ...
      return {
        success: true,
        content: `可用数据库连接：\n${connections
          .map((c) => `- ${c.name} (${c.host}:${c.port}) ${c.active ? '✓ 已连接' : '○ 未连接'}`)
          .join('\n')}`,
    ...
    case 'test': {
      ...
      return {
        ...
        content: result.success ? `连接测试成功：${connection_name}` : `连接测试失败：${result.error}`,
        metadata: result
      }
    }
    ...
    case 'connect': {
      ...
      return {
        success: result.success,
        content: result.success ? `已成功连接到数据库：${connection_name}` : `连接失败：${result.error}`,
        metadata: result
      }
    }
    ...
    case 'disconnect': {
      ...
      return {
        success: result.success,
        content: result.success ? `已断开数据库连接：${connection_name}` : `断开连接失败：${result.error}`,
        metadata: result
      }
    }

    case 'status': {
      const activeConnections = database.getActiveConnections().filter((n) => !n.endsWith('_keepalive'))

      if (connection_name) {
        const isActive = database.isConnectionActive(connection_name)
        return {
          success: true,
          content: isActive ? `连接 "${connection_name}" 当前已连接` : `连接 "${connection_name}" 当前未连接`,
      ...
      return {
        success: true,
        content:
          activeConnections.length > 0 ? `当前活跃连接：${activeConnections.join(', ')}` : '当前没有活跃的数据库连接',
