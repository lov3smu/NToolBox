export default async function handler(params, context) {
  ...
  switch (action) {
    ...
    case 'show_create': {
      const result = await database.executeQuery(connection_name, `SHOW CREATE TABLE \`${table_name}\``, [], {
        autoLimit: false
      })
    ...
    case 'show_indexes': {
      const result = await database.executeQuery(connection_name, `SHOW INDEX FROM \`${table_name}\``, [], {
        autoLimit: false
      })
    ...
    case 'show_columns': {
      const result = await database.executeQuery(connection_name, `SHOW FULL COLUMNS FROM \`${table_name}\``, [], {
        autoLimit: false
      })
