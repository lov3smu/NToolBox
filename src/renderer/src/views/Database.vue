<template>
  <div class="database-container" :style="{ width: windowWidth + 'px' }">
    <header>
      <div class="header-left">
        ...
        <div class="header-shortcuts">
          <div class="shortcut-item" title="新建连接" @click="showAddConnectionModal">
          ...
          <div class="shortcut-item" title="新建查询" @click="createQueryTab('')">
            <svg class="shortcut-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
        ...
        <div class="header-stats">
          <span class="stat-item clickable" title="查看表列表" @click="handleHeaderStatClick('tables')">
          ...
          <span class="stat-item clickable" title="查看视图列表" @click="handleHeaderStatClick('views')">
            <svg class="stat-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
          ...
          <span class="stat-item clickable" title="查看函数列表" @click="handleHeaderStatClick('functions')">
            <svg class="stat-icon fx-icon" viewBox="0 0 32 18" fill="currentColor" stroke="none">
              <text x="0" y="14" font-size="12" font-style="italic" font-weight="600">f</text>
              <text x="14" y="17" font-size="8">(x)</text>
            </svg>
    ...
    <div class="main-content">
      <div class="sidebar" :style="{ width: sidebarWidth + 'px' }">
        ...
        <div class="connection-list" @contextmenu.prevent="showContextMenu($event, 'list')">
          <template v-for="group in connectionGroups" :key="group.name">
            ...
            >
              ...
              <svg class="group-folder-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                <path d="M22 19a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h5l2 3h9a2 2 0 0 1 2 2z" />
              </svg>
              <span class="group-name">{{ group.name }}</span>
              <span class="group-count">{{ group.connections.length }}</span>
            </div>
            <div v-show="expandedGroups.includes(group.name)" class="group-connections">
              <div class="connection-wrapper">
                <div v-for="conn in group.connections" :key="conn.name" class="connection-container">
                  <div
                    :class="[
                      'connection-item',
                      {
                        active: selectedConnection?.name === conn.name,
                        dragging: draggingConnection === conn.name
                      }
                    ]"
                    draggable="true"
                  ...
                  >
                    <span class="connection-expand-placeholder">
                      <svg
                        ...
                        :style="{
                          visibility: conn.active && connectionDatabases[conn.name]?.length > 0 ? 'visible' : 'hidden'
                        }"
                        viewBox="0 0 24 24"
                        fill="none"
                        stroke="currentColor"
                  ...
                  <div v-if="conn.active && expandedConnections.includes(conn.name)" class="connection-databases">
                    <div v-for="db in connectionDatabases[conn.name]" :key="db" class="database-wrapper">
                      <div
                        :class="[
                          'database-item',
                          {
                            active:
                              ...
                              selectedDatabase === db &&
                              selectedObject?.type === 'database' &&
                              selectedObject?.name === db
                          }
                        ]"
                        @click.stop="handleSelectConnectionDatabase(conn, db)"
                        @dblclick="openDatabaseFromSidebar(conn, db)"
                        @contextmenu.prevent.stop="
                          showContextMenu($event, 'database', { connection: conn, database: db })
                        "
                      >
                      ...
                      <div v-if="expandedDatabases.includes(`${conn.name}_${db}`)" class="database-objects">
                        <div class="object-category">
                          ...
                          >
                            <svg
                              class="category-icon"
                              :class="{
                                expanded: expandedObjectCategories.includes(`${conn.name}_${db}_tables`)
                              }"
                              viewBox="0 0 24 24"
                              fill="none"
                              stroke="currentColor"
                            ...
                            <span class="category-count">{{
                              databaseObjectsMap[`${conn.name}_${db}`]?.tables?.length || 0
                            }}</span>
                          </div>
                          <div
                          ...
                          >
                            <div
                              ...
                              :class="[
                                'object-item',
                                {
                                  active: selectedObject?.type === 'table' && selectedObject?.name === table
                                }
                              ]"
                              @click="handleSelectConnectionDatabaseTable(conn, db, table)"
                        ...
                        <div class="object-category">
                          ...
                          >
                            <svg
                              class="category-icon"
                              :class="{
                                expanded: expandedObjectCategories.includes(`${conn.name}_${db}_views`)
                              }"
                              viewBox="0 0 24 24"
                              fill="none"
                              stroke="currentColor"
                            ...
                            <span class="category-count">{{
                              databaseObjectsMap[`${conn.name}_${db}`]?.views?.length || 0
                            }}</span>
                          </div>
                          <div
                          ...
                          >
                            <div
                              ...
                              :class="[
                                'object-item',
                                {
                                  active: selectedObject?.type === 'view' && selectedObject?.name === view
                                }
                              ]"
                              @click="handleSelectConnectionDatabaseView(conn, db, view)"
                            >
                              <svg
                        ...
                        <div class="object-category">
                          ...
                          >
                            <svg
                              class="category-icon"
                              :class="{
                                expanded: expandedObjectCategories.includes(`${conn.name}_${db}_functions`)
                              }"
                              viewBox="0 0 24 24"
                              fill="none"
                              stroke="currentColor"
                            ...
                            <span class="category-count">{{
                              databaseObjectsMap[`${conn.name}_${db}`]?.functions?.length || 0
                            }}</span>
                          </div>
                          <div
                          ...
                          >
                            <div
                              ...
                              :class="[
                                'object-item',
                                {
                                  active: selectedObject?.type === 'function' && selectedObject?.name === func
                                }
                              ]"
                              @click="handleSelectConnectionDatabaseFunction(conn, db, func)"
                            >
                              <svg
          ...
          <template v-if="ungroupedConnections.length > 0">
            <div v-for="conn in ungroupedConnections" :key="conn.name" class="connection-container">
              <div
                :class="[
                  'connection-item',
                  {
                    ...
                    <div class="object-category">
                      ...
                      >
                        <div
                          ...
                          @click="handleSelectConnectionDatabaseFunction(conn, db, func)"
                        >
                          <svg class="object-icon function-icon" viewBox="0 0 32 18" fill="currentColor" stroke="none">
                            <text x="0" y="14" font-size="12" font-style="italic" font-weight="600">f</text>
                            <text x="14" y="17" font-size="8">(x)</text>
                          </svg>
                          <span class="object-name">{{ func }}</span>
          ...
          <div v-if="connectionGroups.length === 0 && ungroupedConnections.length === 0" class="empty-list">
            <p>暂无连接</p>
            <p class="hint">右键点击新建连接</p>
          </div>
        </div>
        <div class="sidebar-resize-handle" @mousedown="startSidebarResize" />
      </div>

      <div class="main-panel" :class="{ 'panel-wide': layoutMode === 'three' }">
          ...
          <div class="panel-header">
            <div class="tabs-container">
              ...
              >
                ...
                <button v-if="tab.closable" class="tab-close-btn" @click.stop="closeTab(tab.id)">×</button>
              </div>
            </div>
          </div>

          <div v-if="activeTabId === 'object'" class="object-panel">
            <div v-if="selectedObject" class="object-content">
              <div class="object-header">
                ...
                <div class="object-search">
                  <input v-model="objectSearchKeyword" type="text" placeholder="搜索..." class="search-input" />
                </div>
              </div>
              <div v-if="selectedObject.type === 'database'" class="object-tables">
                <div v-if="objectTableDetails.length === 0" class="object-loading">
                  <p>加载中...</p>
                </div>
                <div v-else-if="filteredTableDetails.length === 0" class="object-empty-list">
              ...
              <div v-else-if="selectedObject.type === 'table'" class="object-detail">
                <div v-if="tableStructure.length === 0" class="object-loading">
                  <p>加载中...</p>
                </div>
                <div v-else class="structure-list">
                  ...
                  <div class="structure-body">
                    <div v-for="field in filteredTableStructure" :key="field.Field" class="structure-row">
              ...
              <div v-else-if="selectedObject.type === 'view'" class="object-detail">
                <div class="view-definition">
                  <p class="definition-label">视图定义:</p>
                  <pre class="definition-code">{{ viewDefinition }}</pre>
                </div>
              </div>
              <div v-else-if="selectedObject.type === 'function'" class="object-detail">
                <div class="function-definition">
                  <p class="definition-label">函数定义:</p>
          ...
          <div v-else class="query-panel">
            ...
            >
              <template v-if="tab.type === 'query'">
                <div class="query-toolbar">
                  <div class="toolbar-left">
                    <div class="toolbar-select-group">
                      ...
                      <select v-model="tab.connectionName" class="toolbar-select" @change="onTabConnectionChange(tab)">
                        <option value="" />
                    ...
                    <div class="toolbar-select-group">
                      ...
                      <select v-model="tab.database" class="toolbar-select" @change="onTabDatabaseChange(tab)">
                        <option value="" />
                  ...
                  <div class="toolbar-right">
                    ...
                    <button class="btn-small btn-secondary btn-icon-text" @click="clearQueryForTab(tab)">
                      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                ...
                <div class="query-body" :class="{ 'has-result': tab.queryResult }">
                  <div class="query-editor-wrapper" :style="{ flex: tab.queryResult ? '0 0 50%' : '1' }">
                    <SqlEditor
                      :ref="
                        (el) => {
                          if (el) tab.editorRef = el
                        }
                      "
                  ...
                  <div v-if="tab.queryResult" class="query-result-wrapper">
                    <div class="query-result">
                      <div class="result-header">
                        <div v-if="tab.queryResults && tab.queryResults.length > 1" class="result-tabs">
                          <div
                        ...
                        <span v-else class="result-title">查询结果</span>
                        <span class="result-info">
                          {{ tab.queryResult.success ? `成功 - 耗时: ${tab.queryResult.duration}ms` : '失败' }}
                        </span>
                      </div>
                      <div v-if="tab.queryResult.success" class="result-content">
                        <div
                        ...
                        >
                          <div class="result-table-scroll">
                            <table class="result-table">
                              <thead>
                                <tr>
                                  <th v-for="field in tab.queryResult.fields" :key="field.name" class="auto-width">
                                    {{ field.name }}
                                  </th>
                                </tr>
                              </thead>
                              <tbody>
                                <tr v-for="(row, index) in tab.queryResult.rows" :key="index">
                                  <td v-for="field in tab.queryResult.fields" :key="field.name">
                        ...
                        <div v-else class="result-empty">{{ tab.queryResult.rowCount }} 行受影响</div>
                      </div>
                    ...
                    <div class="result-footer">
                      <div class="footer-left">
                        <span v-if="tab.queryResult.success && tab.queryResult.rows" class="result-stats">
                          {{ tab.queryResult.totalRows || tab.queryResult.rows.length }} 行
                        </span>
                        <div
                          v-if="
                            tab.queryResult.success &&
                            tab.queryResult.isSelect &&
                            tab.queryResult.rows &&
                        ...
                        >
                          ...
                          <button
                            class="pagination-btn"
                            :disabled="
                              !tab.queryResult.currentPage ||
                              tab.queryResult.currentPage >= (tab.queryResult.totalPages || 1)
                            "
                            title="下一页"
                            @click="nextPage(tab)"
                          >
                      ...
                      <div class="footer-right">
                        ...
                        >
                          <label class="settings-checkbox">
                            <input v-model="autoLimit" type="checkbox" />
                            <span>限制记录</span>
                          </label>
                          <div v-if="autoLimit" class="settings-limit-input">
                            <input v-model.number="limitCount" type="number" class="limit-input" min="1" max="100000" />
                        ...
                        <div
                          :class="[
                            'footer-settings',
                            {
                              active: limitSettingsDialog.show && limitSettingsDialog.tabId === tab.id
                            }
                          ]"
                          title="设置"
              ...
              <template v-else-if="tab.type === 'table'">
                <div class="query-toolbar table-query-toolbar">
                  ...
                  <button class="toolbar-btn" :disabled="tab.executing" title="刷新" @click="executeTableQuery(tab)">
                    <svg
                      class="toolbar-btn-icon"
                      viewBox="0 0 24 24"
                      fill="none"
                  ...
                  <button class="toolbar-btn" title="导入" @click="importData()">
                    <svg
                      class="toolbar-btn-icon"
                      viewBox="0 0 24 24"
                      fill="none"
                  ...
                  <button class="toolbar-btn" title="导出" @click="exportData()">
                    <svg
                      class="toolbar-btn-icon"
                      viewBox="0 0 24 24"
                      fill="none"
                ...
                <div class="query-body table-query-body">
                  <div class="query-result-wrapper full-height">
                    <div class="query-result">
                      <div v-if="tab.executing" class="result-loading">
                        <div class="loading-spinner" />
                        <span>正在查询...</span>
                      </div>
                      <div v-else-if="tab.queryResult" class="result-content">
                        <div v-if="tab.queryResult.success" class="result-body">
                          <div
                                  ...
                                  <tr>
                                    <th
                                      ...
                                      :class="[
                                        'auto-width',
                                        {
                                          'has-sort': tab.sortField === fieldName,
                                          'sort-asc': tab.sortField === fieldName && tab.sortDirection === 'asc',
                                          'sort-desc': tab.sortField === fieldName && tab.sortDirection === 'desc'
                                        }
                                      ]"
                                      draggable="true"
                                    ...
                                    >
                                      <div class="th-content">
                                        ...
                                        <div class="th-actions" @click.stop="showFieldSortMenu($event, tab, fieldName)">
                                          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                          ...
                          <div v-else class="result-empty">表无数据</div>
                        </div>
                        <div v-else class="result-error">
                          {{ tab.queryResult.error }}
                        </div>
                      </div>
                      <div v-else class="result-empty">点击刷新加载表数据</div>
                    </div>
                    <div class="result-footer">
                      <div class="footer-left">
                        <span
      ...
      <div v-if="layoutMode === 'three'" class="info-panel" :style="{ width: infoPanelWidth + 'px' }">
        <div class="info-panel-resize-handle" @mousedown="startInfoPanelResize" />
        <div class="info-panel-toolbar">
          ...
          >
            <svg class="info-tab-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
              <circle cx="12" cy="12" r="10" />
          ...
          >
            <svg class="info-tab-icon ddl-icon" viewBox="0 0 32 16" fill="currentColor" stroke="none">
              <text x="0" y="13" font-size="12" font-weight="600" font-family="Arial, sans-serif">DDL</text>
            </svg>
          </button>
        </div>

        <div v-if="selectedInfoType" class="info-header">
        ...
        <div class="info-panel-content">
          <div v-if="!selectedInfoType" class="info-empty">
            <p>请选择对象查看信息</p>
          </div>

          <div v-else-if="selectedInfoType === 'group'" class="info-content">
            <div class="info-item">
          ...
          <div v-else-if="selectedInfoType === 'connection'" class="info-content">
            <div class="info-item">
            ...
            <div v-if="connectionInfo" class="info-item">
          ...
          <div v-else-if="selectedInfoType === 'database'" class="info-content">
            <div class="info-item">
              <span class="info-label">连接</span>
              <span class="info-value">{{ selectedConnection?.name }}</span>
            </div>
            <div v-if="databaseInfo" class="info-item">
              <span class="info-label">字符集</span>
          ...
          <div v-else-if="selectedInfoType === 'category'" class="info-content info-category-list">
            <div
          ...
          <div v-else-if="selectedInfoType === 'table' && tableInfoMode === 'info'" class="info-content">
            <div class="info-item">
              <span class="info-label">连接</span>
          ...
          >
            <pre class="info-ddl" v-html="highlightSql(tableDDL || '加载中...')" />
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="status-bar">
      <div class="status-left">
        ...
        <template v-if="lastExecutedSql">
          <span class="status-divider">|</span>
          <span class="sql-preview" :title="lastExecutedSql">{{ truncateSql(lastExecutedSql) }}</span>
          <button class="copy-btn" title="复制SQL" @click="copyLastSql">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
      ...
      <div class="status-right">
        <button :class="['layout-btn', { active: layoutMode === 'two' }]" title="双栏布局" @click="layoutMode = 'two'">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
    ...
    <div v-if="showCreateDialog" class="dialog-overlay" @click.self="closeCreateDialog">
      <div class="dialog">
        ...
        <div class="dialog-tabs">
          <div :class="['dialog-tab', { active: createDialogTab === 'basic' }]" @click="createDialogTab = 'basic'">
            常规
          </div>
          <div
        ...
        <div class="dialog-content">
          <template v-if="createDialogTab === 'basic'">
            <div class="form-inline-group">
              ...
              <input v-model="newConnection.name" type="text" class="input-field-inline" placeholder="连接名称" />
            </div>
            <div class="form-inline-group">
            ...
            <div class="form-inline-group">
              <label class="checkbox-label-inline">
                <input v-model="newConnection.savePassword" type="checkbox" />
                <span>保存密码</span>
              </label>
            </div>
          </template>
          <template v-if="createDialogTab === 'advanced'">
            <div class="form-inline-group">
              ...
              <select v-model="newConnection.charset" class="input-field-inline">
            ...
            <div class="form-inline-group">
              <label class="checkbox-label-inline">
                <input v-model="newConnection.enableKeepAlive" type="checkbox" />
                <span>保持连接间隔（秒）</span>
              </label>
              <input
            ...
            <div class="form-inline-group">
              <label class="checkbox-label-inline">
                <input v-model="newConnection.enableConnectTimeout" type="checkbox" />
                <span>连接超时（秒）</span>
              </label>
              <input
            ...
            <div class="form-inline-group">
              <label class="checkbox-label-inline">
                <input v-model="newConnection.enableReadTimeout" type="checkbox" />
                <span>读取超时（秒）</span>
              </label>
              <input
            ...
            <div class="form-inline-group">
              <label class="checkbox-label-inline">
                <input v-model="newConnection.enableWriteTimeout" type="checkbox" />
                <span>写入超时（秒）</span>
              </label>
              <input
            ...
            <div class="form-inline-group">
              <label class="checkbox-label-inline">
                <input v-model="newConnection.autoConnect" type="checkbox" />
                <span>自动连接</span>
              </label>
            </div>
          </template>
        ...
        <div class="dialog-footer">
          <button class="btn-small btn-secondary" :disabled="testingNewConn" @click="closeCreateDialog">取消</button>
          <button class="btn-small" :disabled="testingNewConn" @click="testNewConnection">
            {{ testingNewConn ? '测试中...' : '测试连接' }}
          </button>
          <button class="btn-small" :disabled="testingNewConn" @click="saveConnection">保存</button>
        </div>
      </div>
    </div>
    ...
    <div v-if="showEditDialog" class="dialog-overlay" @click.self="closeEditDialog">
      <div class="dialog">
        ...
        <div class="dialog-tabs">
          <div :class="['dialog-tab', { active: editDialogTab === 'basic' }]" @click="editDialogTab = 'basic'">
            常规
          </div>
          <div :class="['dialog-tab', { active: editDialogTab === 'advanced' }]" @click="editDialogTab = 'advanced'">
            高级
          </div>
        </div>
        <div class="dialog-content">
          <template v-if="editDialogTab === 'basic'">
            <div class="form-inline-group">
              <label class="form-label-inline">连接名称</label>
              <input v-model="editingConnection.name" type="text" class="input-field-inline" placeholder="连接名称" />
            </div>
            <div class="form-inline-group">
              <label class="form-label-inline">所属分组</label>
              <select v-model="editingConnection.group" class="input-field-inline">
            ...
            <div class="form-inline-group">
              <label class="form-label-inline">主机地址</label>
              <input v-model="editingConnection.host" type="text" class="input-field-inline" placeholder="localhost" />
            </div>
            <div class="form-inline-group">
              <label class="form-label-inline">端口</label>
              <input
            ...
            <div class="form-inline-group">
              <label class="checkbox-label-inline">
                <input v-model="editingConnection.savePassword" type="checkbox" />
                <span>保存密码</span>
              </label>
            </div>
          </template>
          <template v-if="editDialogTab === 'advanced'">
            <div class="form-inline-group">
              <label class="form-label-inline">客户端字符集</label>
              <select v-model="editingConnection.charset" class="input-field-inline">
                <option value="">自动</option>
                <option value="utf8mb4">utf8mb4</option>
                <option value="utf8">utf8</option>
                <option value="latin1">latin1</option>
            ...
            <div class="form-inline-group">
              <label class="checkbox-label-inline">
                <input v-model="editingConnection.enableKeepAlive" type="checkbox" />
                <span>保持连接间隔（秒）</span>
              </label>
              <input
            ...
            <div class="form-inline-group">
              <label class="checkbox-label-inline">
                <input v-model="editingConnection.enableConnectTimeout" type="checkbox" />
                <span>连接超时（秒）</span>
              </label>
              <input
            ...
            <div class="form-inline-group">
              <label class="checkbox-label-inline">
                <input v-model="editingConnection.enableReadTimeout" type="checkbox" />
                <span>读取超时（秒）</span>
              </label>
              <input
            ...
            <div class="form-inline-group">
              <label class="checkbox-label-inline">
                <input v-model="editingConnection.enableWriteTimeout" type="checkbox" />
                <span>写入超时（秒）</span>
              </label>
              <input
            ...
            <div class="form-inline-group">
              <label class="checkbox-label-inline">
                <input v-model="editingConnection.autoConnect" type="checkbox" />
                <span>自动连接</span>
              </label>
            </div>
          </template>
        ...
        <div class="dialog-footer">
          <button class="btn-small" :disabled="testingEditConn" @click="testEditConnection">
            {{ testingEditConn ? '测试中...' : '测试连接' }}
          </button>
          <div class="dialog-footer-right">
            <button class="btn-small btn-secondary" :disabled="testingEditConn" @click="closeEditDialog">取消</button>
            <button class="btn-small" :disabled="testingEditConn" @click="updateConnection">保存</button>
          </div>
        </div>
      </div>
    </div>
      ...
      <div class="dialog">
        ...
        <div class="dialog-content">
          <div class="form-group">
            <label>分组名称</label>
            <input v-model="newGroupName" type="text" class="input-field" placeholder="分组名称" />
          </div>
        </div>
        <div class="dialog-footer">
          <button class="btn-small btn-secondary" @click="closeGroupDialog">取消</button>
          <button class="btn-small" @click="saveGroup">保存</button>
        </div>
      </div>
    </div>
        ...
        <div class="dialog-content manage-content">
          <div class="manage-list">
            <div class="manage-header">
              <span>分组管理</span>
              <button class="btn-small" @click="openNewGroupDialog">新建分组</button>
            </div>
            <div class="manage-groups">
              <div v-for="group in connectionGroups" :key="group.name" class="manage-group-item">
                <span>{{ group.name }} ({{ group.connections.length }}个连接)</span>
                <div class="manage-group-actions">
                  <button class="btn-icon-sm" title="编辑" @click="editGroup(group)">
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                      <path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7" />
                      <path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z" />
                    </svg>
                  </button>
                  <button class="btn-icon-sm btn-danger" title="删除" @click="deleteGroup(group.name)">
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                      <polyline points="3 6 5 6 21 6" />
                      <path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2" />
                    </svg>
              ...
              <div v-if="connectionGroups.length === 0" class="manage-empty">暂无分组</div>
            </div>
          </div>
        </div>
      </div>
    ...
    >
      <template v-if="contextMenu.type === 'header'">
        <div class="menu-item" @click="handleNewConnectionAndHide()">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
        ...
        <div class="menu-item" @click="handleOpenManageAndHide()">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
            <circle cx="12" cy="12" r="3" />
            <path
              d="M19.4 15a1.65 1.65 0 0 0 .33 1.82l.06.06a2 2 0 0 1 0 2.83 2 2 0 0 1-2.83 0l-.06-.06a1.65 1.65 0 0 0-1.82-.33 1.65 1.65 0 0 0-1 1.51V21a2 2 0 0 1-2 2 2 2 0 0 1-2-2v-.09A1.65 1.65 0 0 0 9 19.4a1.65 1.65 0 0 0-1.82.33l-.06.06a2 2 0 0 1-2.83 0 2 2 0 0 1 0-2.83l.06-.06a1.65 1.65 0 0 0 .33-1.82 1.65 1.65 0 0 0-1.51-1H3a2 2 0 0 1-2-2 2 2 0 0 1 2-2h.09A1.65 1.65 0 0 0 4.6 9a1.65 1.65 0 0 0-.33-1.82l-.06-.06a2 2 0 0 1 0-2.83 2 2 0 0 1 2.83 0l.06.06a1.65 1.65 0 0 0 1.82.33H9a1.65 1.65 0 0 0 1-1.51V3a2 2 0 0 1 2-2 2 2 0 0 1 2 2v.09a1.65 1.65 0 0 0 1 1.51 1.65 1.65 0 0 0 1.82-.33l.06-.06a2 2 0 0 1 2.83 0 2 2 0 0 1 0 2.83l-.06.06a1.65 1.65 0 0 0-.33 1.82V9a1.65 1.65 0 0 0 1.51 1H21a2 2 0 0 1 2 2 2 2 0 0 1-2 2h-.09a1.65 1.65 0 0 0-1.51 1z"
        ...
        <div class="menu-item" @click="handleCloseAllConnectionsAndHide()">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
        ...
        <div class="menu-item" @click="handleOpenNewGroupAndHide()">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
            <path d="M22 19a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h5l2 3h9a2 2 0 0 1 2 2z" />
        ...
        <div class="menu-divider" />
        <div class="menu-item" @click="handleLoadConnectionsAndHide()">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
            <polyline points="23 4 23 10 17 10" />
      ...
      <template v-if="contextMenu.type === 'connection'">
        <div v-if="!contextMenu.target.active" class="menu-item" @click="handleConnectAndHide()">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
            <path d="M18.36 6.64a9 9 0 1 1-12.73 0" />
            <line x1="12" y1="2" x2="12" y2="12" />
          </svg>
        ...
        <div v-if="contextMenu.target.active" class="menu-item" @click="handleDisconnectAndHide()">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
            <path d="M18.36 6.64a9 9 0 1 1-12.73 0" />
            <line x1="12" y1="2" x2="12" y2="12" />
          </svg>
        ...
        <div class="menu-divider" />
        <div class="menu-item" @click="handleNewConnectionInGroupAndHide()">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
            <line x1="12" y1="5" x2="12" y2="19" />
            <line x1="5" y1="12" x2="19" y2="12" />
          </svg>
        ...
        <div class="menu-divider" />
        <div class="menu-item" @click="handleEditConnectionAndHide()">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
            <path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7" />
            <path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z" />
          </svg>
        ...
        <div class="menu-item" @click="handleDuplicateConnectionAndHide()">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
            <rect x="9" y="9" width="13" height="13" rx="2" ry="2" />
            <path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1" />
          </svg>
        ...
        <div class="menu-item danger" @click="handleDeleteConnectionAndHide()">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
            <polyline points="3 6 5 6 21 6" />
            <path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2" />
          </svg>
        ...
        <div class="menu-item has-submenu">
          ...
          <div class="submenu">
            <div class="menu-item" @click.stop="handleOpenNewGroupAndHide()">
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                <path d="M22 19a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h5l2 3h9a2 2 0 0 1 2 2z" />
                <line x1="12" y1="11" x2="12" y2="17" />
                <line x1="9" y1="14" x2="15" y2="14" />
            ...
            <div
              ...
              @click.stop="handleRemoveFromGroupAndHide()"
            >
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                <polyline points="3 6 5 6 21 6" />
            ...
            <div class="menu-divider" />
            <div class="submenu-header">移动到</div>
            <div
              v-for="g in groups"
              :key="g"
              :class="['menu-item', { disabled: g === contextMenu.target.group }]"
              @click.stop="handleMoveToGroupAndHide(g)"
            >
              {{ g }}
            </div>
            <div v-if="groups.length === 0" class="menu-item disabled">暂无分组</div>
          </div>
        </div>
        <div class="menu-divider" />
        <div class="menu-item" @click="handleLoadConnectionsAndHide()">
      ...
      <template v-if="contextMenu.type === 'group'">
        <div class="menu-item" @click="handleNewConnectionInNamedGroupAndHide()">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
            <line x1="12" y1="5" x2="12" y2="19" />
            <line x1="5" y1="12" x2="19" y2="12" />
          </svg>
        ...
        <div class="menu-divider" />
        <div class="menu-item" @click="handleEditGroupAndHide()">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
            <path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7" />
            <path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z" />
          </svg>
        ...
        <div class="menu-item danger" @click="handleDeleteGroupAndHide()">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
            <polyline points="3 6 5 6 21 6" />
            <path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2" />
          </svg>
      ...
      <template v-if="contextMenu.type === 'database'">
        <div
          ...
          class="menu-item"
          @click="handleOpenDatabaseAndHide()"
        >
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
            <ellipse cx="12" cy="5" rx="9" ry="3" />
            <path d="M21 12c0 1.66-4 3-9 3s-9-1.34-9-3" />
        ...
        <div
          v-if="openedDatabases.includes(`${contextMenu.target.connection.name}_${contextMenu.target.database}`)"
          class="menu-item"
          @click="handleCloseDatabaseAndHide()"
        >
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
            <path d="M18.36 6.64a9 9 0 1 1-12.73 0" />
            <line x1="12" y1="2" x2="12" y2="12" />
        ...
        <div class="menu-divider" />
        <div class="menu-item" @click="handleEditDatabaseAndHide()">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
            <path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7" />
            <path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z" />
          </svg>
        ...
        <div class="menu-item danger" @click="handleDeleteDatabaseAndHide()">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
            <polyline points="3 6 5 6 21 6" />
            <path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2" />
          </svg>
        ...
        <div class="menu-divider" />
        <div class="menu-item" @click="handleNewQueryForDatabaseAndHide()">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
            <path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z" />
            <polyline points="14 2 14 8 20 8" />
            <line x1="16" y1="13" x2="8" y2="13" />
        ...
        <div class="menu-item" @click="handleRunSqlFileAndHide()">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
            <path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z" />
            <polyline points="14 2 14 8 20 8" />
            <polyline points="10 12 8 12 8 16" />
            <polyline points="16 12 14 12 14 16" />
          </svg>
          运行SQL文件
        </div>
        <div class="menu-item has-submenu">
          ...
          <div class="submenu">
            <div class="menu-item" @click.stop="handleDumpDatabaseStructureAndHide()">结构和数据</div>
            <div class="menu-item" @click.stop="handleDumpDatabaseDataAndHide()">仅数据</div>
          </div>
        </div>
        <div class="menu-divider" />
        <div class="menu-item" @click="handleRefreshDatabaseAndHide()">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
            <polyline points="23 4 23 10 17 10" />
            <path d="M20.49 15a9 9 0 1 1-2.12-9.36L23 10" />
          </svg>
      ...
      <template v-if="contextMenu.type === 'category'">
        <div class="menu-item" @click="handleRefreshCategoryAndHide()">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
            <polyline points="23 4 23 10 17 10" />
            <path d="M20.49 15a9 9 0 1 1-2.12-9.36L23 10" />
          </svg>
          刷新{{
            contextMenu.target.category === 'tables' ? '表' : contextMenu.target.category === 'views' ? '视图' : '函数'
          }}列表
        </div>
      </template>
      <template v-if="contextMenu.type === 'table'">
        <div class="menu-item" @click="handleOpenNewQueryAndHide()">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
            <rect x="3" y="3" width="18" height="18" rx="2" />
            <line x1="3" y1="9" x2="21" y2="9" />
            <line x1="9" y1="21" x2="9" y2="9" />
        ...
        <div class="menu-divider" />
        <div class="menu-item danger" @click="handleDeleteTableAndHide()">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
            <polyline points="3 6 5 6 21 6" />
            <path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2" />
          </svg>
        ...
        <div class="menu-item danger" @click="handleOpenTableAndHide()">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
            <path d="M3 6h18" />
        ...
        <div class="menu-item danger" @click="handleTruncateTableAndHide()">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
            <path d="M3 6h18" />
            <path d="M8 6V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2" />
            <rect x="8" y="11" width="8" height="10" />
          </svg>
          截断表
        </div>
        <div class="menu-divider" />
        <div class="menu-item has-submenu">
          ...
          <div class="submenu">
            <div class="menu-item" @click.stop="handleCopyTableStructureDataAndHide()">结构和数据</div>
            <div class="menu-item" @click.stop="handleCopyTableStructureAndHide()">仅结构</div>
          </div>
        </div>
        <div class="menu-item has-submenu">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
          ...
          <div class="submenu">
            <div class="menu-item" @click.stop="handleDumpTableStructureDataAndHide()">结构和数据</div>
            <div class="menu-item" @click.stop="handleDumpTableStructureAndHide()">仅结构</div>
          </div>
        </div>
        <div class="menu-divider" />
        <div class="menu-item" @click="handleEmptyTableAndHide()">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
            <path d="M17 3a2.85 2.83 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5L17 3z" />
          </svg>
          重命名
        </div>
        <div class="menu-item" @click="handleRenameTableAndHide()">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
            <polyline points="23 4 23 10 17 10" />
            <path d="M20.49 15a9 9 0 1 1-2.12-9.36L23 10" />
          </svg>
    ...
    <div
      ...
      @click.stop
    >
      <div class="menu-item" @click="handleRefreshTableAndHide()">
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
      ...
      <div class="menu-item" @click="closeOtherTabs(tabContextMenu.tab.id)">
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
          <rect x="3" y="3" width="18" height="18" rx="2" />
      ...
      <div class="menu-item" @click="closeTabsToRight(tabContextMenu.tab.id)">
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
          <polyline points="9 18 15 12 9 6" />
      ...
      <div class="menu-item" @click="closeTabsToLeft(tabContextMenu.tab.id)">
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
          <polyline points="15 18 9 12 15 6" />
    ...
    <div
      ...
      @click.stop
    >
      <div class="menu-item" @click="sortFieldAsc()">
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
          <polyline points="18 15 12 9 6 15" />
        </svg>
        升序排序
      </div>
      <div class="menu-item" @click="sortFieldDesc()">
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
      ...
      <div class="menu-divider" />
      <div class="menu-item" @click="clearFieldSort()">
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
          <line x1="18" y1="6" x2="6" y2="18" />
          <line x1="6" y1="6" x2="18" y2="18" />
        </svg>
    ...
    <div v-if="promptDialog.visible" class="dialog-overlay" @click.self="onPromptCancel">
      <div class="dialog dialog-small">
        <div class="dialog-header">
          <h3>{{ promptDialog.title }}</h3>
          <button class="close-btn" @click="onPromptCancel">×</button>
        </div>
        <div class="dialog-content">
          <div class="form-group">
            <input
        ...
        <div class="dialog-footer">
          <button class="btn btn-secondary" @click="onPromptCancel">取消</button>
          <button class="btn btn-primary" @click="onPromptConfirm">确定</button>
        </div>
      </div>
    </div>
  </div>
...
const tabs = ref([{ id: 'object', name: '对象', type: 'object', closable: false }])
...
const filteredTableDetails = computed(() => {
  ...
  return objectTableDetails.value.filter(
    (table) =>
      table.name.toLowerCase().includes(keyword) || (table.comment && table.comment.toLowerCase().includes(keyword))
  )
})
...
const filteredTableStructure = computed(() => {
  ...
  const keyword = objectSearchKeyword.value.toLowerCase()
  return tableStructure.value.filter(
    (field) => field.Field.toLowerCase().includes(keyword) || field.Type.toLowerCase().includes(keyword)
  )
})
...
async function loadDatabaseObjects(connName, dbName) {
  ...
  try {
    ...
    const viewsResult = await dbExecuteQuery(
      connName,
      `SELECT TABLE_NAME FROM information_schema.VIEWS WHERE TABLE_SCHEMA = '${dbName}'`,
      [],
      { autoLimit: false }
    )
    const functionsResult = await dbExecuteQuery(
      connName,
      `SELECT ROUTINE_NAME FROM information_schema.ROUTINES WHERE ROUTINE_SCHEMA = '${dbName}' AND ROUTINE_TYPE = 'FUNCTION'`,
      [],
      { autoLimit: false }
    )
...
function handleSelectConnectionDatabase(conn, db) {
  selectConnection(conn)
  selectDatabase(db)
  selectDatabaseObject(db)
}

function handleSelectConnectionDatabaseTable(conn, db, table) {
  selectConnection(conn)
  selectDatabase(db)
  selectTableObject(table)
}

function handleSelectConnectionDatabaseView(conn, db, view) {
  selectConnection(conn)
  selectDatabase(db)
  selectViewObject(view)
}

function handleSelectConnectionDatabaseFunction(conn, db, func) {
  selectConnection(conn)
  selectDatabase(db)
  selectFunctionObject(func)
}

function handleOpenNewGroupAndHide() {
  openNewGroupDialog()
  hideContextMenu()
}

function handleRemoveFromGroupAndHide() {
  if (contextMenu.value.target.group) {
    removeFromGroup(contextMenu.value.target)
  }
  hideContextMenu()
}

function handleMoveToGroupAndHide(g) {
  if (g !== contextMenu.value.target.group) {
    moveToGroup(contextMenu.value.target, g)
  }
  hideContextMenu()
}

function handleDumpDatabaseStructureAndHide() {
  dumpDatabase(contextMenu.value.target, 'structure')
  hideContextMenu()
}

function handleDumpDatabaseDataAndHide() {
  dumpDatabase(contextMenu.value.target, 'data')
  hideContextMenu()
}

function handleCopyTableStructureDataAndHide() {
  copyTable(contextMenu.value.target, 'structure_data')
  hideContextMenu()
}

function handleCopyTableStructureAndHide() {
  copyTable(contextMenu.value.target, 'structure')
  hideContextMenu()
}

function handleDumpTableStructureDataAndHide() {
  dumpTable(contextMenu.value.target, 'structure_data')
  hideContextMenu()
}

function handleDumpTableStructureAndHide() {
  dumpTable(contextMenu.value.target, 'structure')
  hideContextMenu()
}

function handleNewConnectionAndHide() {
  newConnection.value.group = ''
  showCreateDialog.value = true
  hideContextMenu()
}

function handleOpenManageAndHide() {
  openManageDialog()
  hideContextMenu()
}

function handleCloseAllConnectionsAndHide() {
  closeAllConnections()
  hideContextMenu()
}

function handleLoadConnectionsAndHide() {
  loadConnections()
  hideContextMenu()
}

function handleConnectAndHide() {
  selectConnection(contextMenu.value.target)
  connectToDatabase()
  hideContextMenu()
}

function handleDisconnectAndHide() {
  selectConnection(contextMenu.value.target)
  disconnectFromDatabase()
  hideContextMenu()
}

function handleNewConnectionInGroupAndHide() {
  newConnection.value.group = contextMenu.value.target.group || ''
  showCreateDialog.value = true
  hideContextMenu()
}

function handleOpenNewQueryAndHide() {
  openNewQuery(contextMenu.value.target)
  hideContextMenu()
}

function handleEditConnectionAndHide() {
  editConnection(contextMenu.value.target)
  hideContextMenu()
}

function handleDuplicateConnectionAndHide() {
  duplicateConnection(contextMenu.value.target)
  hideContextMenu()
}

function handleDeleteConnectionAndHide() {
  selectConnection(contextMenu.value.target)
  deleteConnection()
  hideContextMenu()
}

function handleNewConnectionInNamedGroupAndHide() {
  newConnection.value.group = contextMenu.value.target.name
  showCreateDialog.value = true
  hideContextMenu()
}

function handleEditGroupAndHide() {
  editGroup(contextMenu.value.target)
  hideContextMenu()
}

function handleDeleteGroupAndHide() {
  deleteGroup(contextMenu.value.target.name)
  hideContextMenu()
}

function handleOpenDatabaseAndHide() {
  openDatabase(contextMenu.value.target)
  hideContextMenu()
}

function handleCloseDatabaseAndHide() {
  closeDatabase(contextMenu.value.target)
  hideContextMenu()
}

function handleEditDatabaseAndHide() {
  editDatabase(contextMenu.value.target)
  hideContextMenu()
}

function handleDeleteDatabaseAndHide() {
  deleteDatabase(contextMenu.value.target)
  hideContextMenu()
}

function handleNewQueryForDatabaseAndHide() {
  newQueryForDatabase(contextMenu.value.target)
  hideContextMenu()
}

function handleRunSqlFileAndHide() {
  runSqlFile(contextMenu.value.target)
  hideContextMenu()
}

function handleRefreshDatabaseAndHide() {
  refreshDatabase(contextMenu.value.target)
  hideContextMenu()
}

function handleRefreshCategoryAndHide() {
  refreshCategory(contextMenu.value.target)
  hideContextMenu()
}

function handleOpenTableAndHide() {
  openTable(contextMenu.value.target)
  hideContextMenu()
}

function handleDeleteTableAndHide() {
  deleteTable(contextMenu.value.target)
  hideContextMenu()
}

function handleEmptyTableAndHide() {
  emptyTable(contextMenu.value.target)
  hideContextMenu()
}

function handleTruncateTableAndHide() {
  truncateTable(contextMenu.value.target)
  hideContextMenu()
}

function handleRenameTableAndHide() {
  renameTable(contextMenu.value.target)
  hideContextMenu()
}

function handleRefreshTableAndHide() {
  refreshTable(contextMenu.value.target)
  hideContextMenu()
}
...
async function loadTableStructureForObject(connName, dbName, tableName) {
  ...
  try {
    ...
    const ddlResult = await dbExecuteQuery(connName, `SHOW CREATE TABLE \`${dbName}\`.\`${tableName}\``, [], {
      autoLimit: false
    })
...
async function loadDatabaseInfo(connName, dbName) {
  try {
    const result = await dbExecuteQuery(
      connName,
      `SELECT SCHEMA_NAME, DEFAULT_CHARACTER_SET_NAME, DEFAULT_COLLATION_NAME FROM information_schema.SCHEMATA WHERE SCHEMA_NAME = '${dbName}'`,
      [],
      { autoLimit: false }
    )
    if (result.success && result.rows && result.rows.length > 0) {
...
async function loadTableInfo(connName, dbName, tableName) {
  try {
    const result = await dbExecuteQuery(
      connName,
      `SELECT TABLE_NAME, TABLE_ROWS, ENGINE, AUTO_INCREMENT, ROW_FORMAT, TABLE_COLLATION, CREATE_TIME, UPDATE_TIME, CHECK_TIME, INDEX_LENGTH, DATA_LENGTH, MAX_DATA_LENGTH, DATA_FREE, CREATE_OPTIONS, TABLE_COMMENT FROM information_schema.TABLES WHERE TABLE_SCHEMA = '${dbName}' AND TABLE_NAME = '${tableName}'`,
      [],
      { autoLimit: false }
    )
    if (result.success && result.rows && result.rows.length > 0) {
...
async function loadConnectionInfo(connName) {
  try {
    const result = await dbExecuteQuery(
      connName,
      'SELECT VERSION() AS version, @@character_set_server AS charset, @@collation_server AS collation, @@session.auto_increment_increment AS autoIncrementIncrement',
      [],
      { autoLimit: false }
    )
    if (result.success && result.rows && result.rows.length > 0) {
...
async function viewTableStructure(tableName) {
  ...
  try {
    const result = await dbGetTableStructure(selectedConnection.value.name, selectedDatabase.value, tableName)
...
function closeDatabase(target) {
  ...
  expandedObjectCategories.value = expandedObjectCategories.value.filter((k) => !k.startsWith(`${key}_`))

  const tabsToClose = tabs.value.filter(
    (t) => t.type === 'query' && t.connectionName === target.connection.name && t.database === target.database
  )
...
async function dumpDatabase(target, type) {
  ...
  try {
    ...
    for (const tableName of tablesResult.tables) {
      ...
      if (type === 'data') {
        const dataResult = await dbExecuteQuery(target.connection.name, `SELECT * FROM \`${tableName}\``, [], {
          autoLimit: false
        })
...
async function deleteTable(target) {
  ...
  const result = await executeSqlDirect(
    `DROP TABLE \`${target.database}\`.\`${target.table}\`;`,
...
async function emptyTable(target) {
  ...
  const result = await executeSqlDirect(
    `DELETE FROM \`${target.database}\`.\`${target.table}\`;`,
    target.connection.name
  )
...
async function truncateTable(target) {
  ...
  const result = await executeSqlDirect(
    `TRUNCATE TABLE \`${target.database}\`.\`${target.table}\`;`,
    target.connection.name
  )
...
async function copyTable(target, type) {
  ...
  if (type === 'structure') {
    const result = await executeSqlDirect(
      `CREATE TABLE \`${target.database}\`.\`${newTableName}\` LIKE \`${target.database}\`.\`${target.table}\`;`,
      target.connection.name
    )
    ...
    const createResult = await executeSqlDirect(
      `CREATE TABLE \`${target.database}\`.\`${newTableName}\` LIKE \`${target.database}\`.\`${target.table}\`;`,
      target.connection.name
    )
    ...
    const insertResult = await executeSqlDirect(
      `INSERT INTO \`${target.database}\`.\`${newTableName}\` SELECT * FROM \`${target.database}\`.\`${target.table}\`;`,
      target.connection.name
    )
...
async function dumpTable(target, type) {
  ...
  try {
    ...
    if (type === 'structure_data') {
      const dataResult = await dbExecuteQuery(target.connection.name, `SELECT * FROM \`${target.table}\``, [], {
        autoLimit: false
      })
...
async function renameTable(target) {
  ...
  const result = await executeSqlDirect(
    `RENAME TABLE \`${target.database}\`.\`${target.table}\` TO \`${target.database}\`.\`${newTableName}\`;`,
    target.connection.name
  )
